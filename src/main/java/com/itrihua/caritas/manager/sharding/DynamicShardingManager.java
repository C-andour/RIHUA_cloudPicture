...
public class DynamicShardingManager {
    ...
    private void updateShardingTableNodes() {
        ...
        String newActualDataNodes = tableNames.stream()
                .map(tableName -> "rihua_caritas_backend." + tableName) // 确保前缀合法
        ...
        if (shardingRule.isPresent()) {
            ...
            List<ShardingTableRuleConfiguration> updatedRules = ruleConfig.getTables()
                    .stream()
                    .map(oldTableRule -> {
                        //获取名为picture的规则, 即配置文件里面(可能有多个)
                        if (LOGIC_TABLE_NAME.equals(oldTableRule.getLogicTable())) {
                            //获取当前规则,并修改actual-data-nodes,其他不变
                            ShardingTableRuleConfiguration newTableRuleConfig = new ShardingTableRuleConfiguration(LOGIC_TABLE_NAME, newActualDataNodes);
                            newTableRuleConfig.setDatabaseShardingStrategy(oldTableRule.getDatabaseShardingStrategy()); //原分库策略
                            newTableRuleConfig.setTableShardingStrategy(oldTableRule.getTableShardingStrategy()); //原分表策略
                            newTableRuleConfig.setKeyGenerateStrategy(oldTableRule.getKeyGenerateStrategy()); //主键生成策略
                            newTableRuleConfig.setAuditStrategy(oldTableRule.getAuditStrategy()); //设置原有审计策略
